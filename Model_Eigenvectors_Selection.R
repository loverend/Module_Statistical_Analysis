...
model_selection <- function(eigenvectors, metadata, outputdir,type_receptor, metahealth){
	...
	for(i in 1:(length(colnames(eigenvectors))-3)){
		...
		multiplot(plotlist = myplots2, cols = 4)
		dev.off()	
		...
		pdf(paste0(outputdir,"/Model_Assessment_SPREAD.pdf"), width=12, height=12)
		plot(ggplot(all_models, aes(x=AICc, fill=Modnames))+geom_histogram(binwidth=1, colour="black")+theme_classic()+facet_wrap(~Module, scales="free", ncol=4)+ylab("Count")+labs(fill="Model")+ggtitle("Model Selection using AICc")+guides(fill=guide_legend(ncol=1)))
		dev.off()
